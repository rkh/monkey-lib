...
Currently it does *not* run on MacRuby, but I'm working on it. (Meaning: The specs are not working.
Manual testing hints it might just work fine with Backports.)
...
with your ruby). You can use one ruby implementation to run the specs on another one. Fo instance, I use rvm to manage my rubies:
  ...
  rvm use default
  RUBY=$rvm_path/.rvm/rubies/macruby-0.5/bin/ruby SPEC_RUNNER=mspec rake spec:backports
